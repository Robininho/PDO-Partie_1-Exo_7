      ...
      <div class="col-sm">
        <div class="card" style="width: 18rem;">
          ...
          <div class="card-body">
            <p class="card-text">
            <?php
            $sql = 'SELECT *, CASE WHEN card = 1 then "Oui"
            WHEN card = 0 then "Non" end as own FROM clients';
            $req = $pdo->query($sql); 
            while( $row = $req->fetch() ) {
              echo 'Nom : ' . $row['lastName'] . '<br/>' . 'Prénom : ' . $row['firstName'] .
                    '<br/>' . 'Date de naissance : ' . $row['birthDate'] .
                      '<br/>' . 'Carte de fidélité : ' . $row['own'] .
                       '<br/>';  
